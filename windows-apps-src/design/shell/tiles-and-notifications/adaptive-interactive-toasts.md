...
To enable a quick reply text box, like for a messaging scenario, add a text input and a button, and reference the text input's id so that the button is displayed adjacent to the input. The icon for the button should be white transparent 32x32 pixel images at 100% scaling, and should have no padding included in the image itself.
